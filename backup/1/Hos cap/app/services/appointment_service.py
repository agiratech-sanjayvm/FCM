...
async def accept_appointment(
    ...
    # ── Step 1: Begin transaction (implicit via session) ──────────────
    async with db.begin():

        # ── Step 2: Lock the row with SELECT FOR UPDATE ──────────────
        # This prevents any other concurrent transaction from reading
        # or modifying this row until we commit or rollback.
        stmt = (
            select(Appointment)
        ...
        if appointment.status != AppointmentStatus.PENDING:
            # Another doctor already accepted — the lock guarantees this
            # check is atomic and not subject to race conditions.
            raise ValueError(
                f"Appointment {appointment_id} is already accepted. "
                "Only the first doctor can accept."
            )

        # ── Step 4: Mutate and commit (auto-committed by `db.begin()`) ─
        appointment.status = AppointmentStatus.ACCEPTED
        appointment.doctor_id = doctor_id
        appointment.accepted_at = datetime.now(timezone.utc)

    # Transaction is now committed; the row lock is released.
